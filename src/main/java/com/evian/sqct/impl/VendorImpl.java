...
public class VendorImpl implements IVendorDao {
	...
	public String doorReplenishment(final Integer eid,final Integer mainboardId,
		...
		String result = null;
		if(account==null) {
			result = (String)vendorDataSource.execute(
...
//							System.out.println("count = "+cs.getObject("Count"));
	...
	@SuppressWarnings({"unchecked", "rawtypes"})
	@Override
	public Map<String, Object> Proc_Backstage_vendor_order_select_groupByMainboardIdOrProductId(final Integer statisticsType, final String beginTime, final String endTime
			, final String eName, final Integer eid, final String mainboardNo, final Integer orderStatus, final Integer dataSourse, final Boolean isPay, final String productName
			, final String shopName, final Integer doorIndex, final Boolean isTest, final Integer buyTimesMoreThen, final String cellphone, final Integer PageIndex
			, final Integer PageSize, final Boolean IsSelectAll) {
		Map<String, Object> result = (Map<String, Object>) vendorDataSource.execute(
				"{call Proc_Backstage_vendor_order_select_groupByMainboardIdOrProductId(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}",
				new CallableStatementCallback() {
	...
	@SuppressWarnings({"unchecked", "rawtypes"})
	@Override
	public Map<String, Object> Proc_Backstage_vendor_replenishment_select(final String beginTime, final String endTime
			, final Integer eid, final String account, final String productCode, final String productName, final Boolean isTest
			, final String containerCode, final Integer doorIndex, final Integer PageIndex, final Integer PageSize, final Boolean IsSelectAll) {
		Map<String, Object> result = (Map<String, Object>)vendorDataSource.execute(
				"{call Proc_Backstage_vendor_replenishment_select(?,?,?,?,?,?,?,?,?,?,?,?,?)}",
				new CallableStatementCallback() {
					public Object doInCallableStatement(CallableStatement cs)
							throws SQLException {
						cs.setObject("beginTime", beginTime);
						...
						try {
							List<Map<String,Object>> result = ResultSetToBeanHelper.resultSetToList(rs, Map.class);
							map.put("Count", cs.getInt("Count"));
							map.put("replenishmentRecord", result);
							return map;
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
